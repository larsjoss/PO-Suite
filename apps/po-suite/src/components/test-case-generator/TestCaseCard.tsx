...
import { Alert, Card, CardContent, CardFooter } from '../../shared/components';
...
export function TestCaseCard({ testCase: tc }: Props) {
  ...
  return (
    <Card as="li" className={hasFlag ? 'bg-amber-50 border-amber-300' : ''}>
      <CardContent className="space-y-4">
        ...
        {/* Flag-Callout */}
        {tc.flag && (
          <Alert variant="warning" title={FLAG_TYPE_LABELS[tc.flag.type] ?? tc.flag.type}>
            {tc.flag.message}
          </Alert>
        )}
